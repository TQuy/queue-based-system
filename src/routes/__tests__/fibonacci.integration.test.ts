...
describe('Fibonacci Routes E2E Tests - Without Decoupled Workers', () => {
    let app: Application;
    ...
    afterAll(async () => {
        try {
            ...
            // await dataStoreService.disconnect(); // Keep connected for integration tests(); // Keep connected for integration tests
        } catch (error) {
    ...
    it('should be able to poll for result of scheduled fibonacci task', async () => {
        ...
        const received = new Promise<any>((resolve, reject) => {
            ...
            clientSocket.on(FIBONACCI_WS_FAILED_EVENT, (data: any) => {
                ...
                reject(new Error(`[socket.io] Received failed event: ${JSON.stringify(data)}`));
            });
        });
...
describe('Fibonacci Routes E2E Tests - With Decoupled Workers', () => {
    let app: Application;
    let dataStoreService: DatastoreService;
    let messageBrokerService: MessageBrokerService;
    let httpServer: http.Server;
    ...
    it.only('should be able to poll for result of scheduled fibonacci task with decoupled workers', async () => {
        const input = 7;
        const expectedResult = 13; // F(7)
